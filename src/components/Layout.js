...
export default function Layout({ children }) {
  const renderServiceList = () => {
    return config.serviceList.map((item, index) => {
      return (
        <p
          key={index}
          className={clsx(styles.navFooterItem, styles.navFooterContactItem)}
        >
          {item}
        </p>
      );
  ...
  return (
    ...
    >
      ...
      <footer className="w-full bg-neutral-950">
        ...
        <div
          className={clsx("flex items-center justify-center bg-black h-[50px]")}
        >
          <p className={clsx("text-sm text-gray-200")}>
            Copyright © 2024 - Sắc Việt Entertainment. All Rights Reserved.
          </p>
        </div>
      </footer>
    </div>
